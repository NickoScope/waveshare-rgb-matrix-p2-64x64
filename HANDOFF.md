...
- **Где:** `enclosure/acrylic/`. Печатный проект в `enclosure/cad/` не тронут.
- **Главное за смену технологии.** Вся FDM-конструкция построена вокруг того,
  что деталь 284 мм не лезет на столик 250 и корпус режется на шесть частей.
  Лазеру это безразлично: лицевая плита одним куском, шва по лицу нет.
- **Габарит 334,6 x 177,8 x 33.** Семь деталей, рез 988 см.
  - Матрицы **утоплены в окно заподлицо** с лицом: их первые 4 мм сидят в
    передней плите, поэтому она не добавляется к глубине.
  - Между матрицами **нет ребра** (ТЗ §4.1). Положение обеих задаёт задняя
    плита: 12 стоек M3 через штатные отверстия, размах 113,7.
  - **Высота борта = полость + 8** — шипы проходят сквозь обе плиты. Это
    место дважды вызвало путаницу, вынесено отдельным разделом в README.
  - Глубина: полость 25, динамик 28 заходит в толщину задней плиты на 3 мм
    через окно ⌀26 и кончается за 1 мм до наружной грани. Наружу не торчит.
    Перебрали 28/20, 29/21, 37/29 — все записаны в README с причиной отказа.
- **Лицевая панель.** Динамик Visaton FRWS 5 SC по оси экрана, энкодер по
  линии надписи. Решётка динамика и надпись «NickoScope Matrix» набраны ОДНОЙ
  точечной сеткой ⌀2 с шагом 3,2 — по мотивам самой матрицы.
  - Строка шрифтом 4x6: 80 колонок, 153 отверстия, ширина ровно 256,0 —
    в ширину экрана. Влезает в 128 колонок, то есть и на само табло.
  - Она же **продух**: 481 мм².
  - Надписи внутри решётки динамика невозможны: круг ⌀45 — это 11 колонок,
    строке нужно 80.
- **Задняя плита:** два ряда щелей вразбежку, окно под магнит, USB-C, 12 стоек.
  Щели, попадающие на стойки, выбрасываются автоматически — между верхом
  стоек (158,0) и пазом верхнего борта (166,8) всего 8,8 мм.
- **3D и просмотрщик:** https://claude.ai/artifact/7sWU4i17Dzby5VpN64sUPF
  Заложены грабли прошлого корпуса: допуск сетки 0,06 вместо микронного,
  `mesh.vertex_normals` до экспорта (иначе three.js рисует всё чёрным),
  `.loading[hidden]{display:none}`, объявление кодировки.
- **Нашёл дыру в собственной проверке:** сравнивались только окружности между
  собой, поэтому зазор «щель вентиляции против стойки» не проверялся вовсе.
  Добавлено сравнение между разными группами резов.

**РЕЗАТЬ НЕЛЬЗЯ** — семь величин не обмерены, `audit.py` печатает список на
каждом прогоне. Главная — **кромка платы матрицы**: чертёж размеряет литую
рамку 127,8 и плату не размеряет вовсе; если наружу выступает рамка, шаг через
стык выйдет 1,8 вместо 2,0. Плюс крепёж контроллера, у Waveshare не размерен.

**Открыто:** чем закрыть лицо — прозрачный акрил или дымчатый. Рекомендация:
дымчатый (свет диода проходит фильтр один раз, внешний дважды, чёрное глубже).
Геометрия от выбора не зависит.
