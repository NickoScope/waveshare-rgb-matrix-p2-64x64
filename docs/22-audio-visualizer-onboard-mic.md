...
| D3 | **Done in code 2026-09-16, `5eef2b0`, not yet on the panel.** MQTT reconnect held `loop()` 3 s every 5 s during an outage | `[loop] mqtt took 3001 ms` x ~40 | Connect timeout 1 s (`WiFiClient::setTimeout`, seconds, `WiFiClient.cpp:325`); the gap doubles 5 -> 60 s and resets on a connect; the host is resolved once, an address as it is and a `.local` name through mDNS with a 2 s bound, because `WiFiClient::connect()` resolves with no timeout of its own and a dead name cost ~15 s a try; three failures in a row drop the cached address. Verify on hardware: at most one ~1 s stall per gap while the broker is away |
