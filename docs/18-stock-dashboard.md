...
**Also done at 17:10:**
- The flag matrix on b40c3d6 passed: 39 of 39 rows behaved as intended.
- The HA dashboard "Биржа" has a new view, **"Портфель"** (`/stock-market/portfolio`). It was generated by `matrix_market.cli view`, with two changes:
  - the `_comment` key was dropped;
  - the Easy Stock card was corrected to `entities` and `default_range`.
- All 66 entity ids the view uses exist among the app's 68 discovered sensors.

**Backlog found on the real system:**
- `live: shrunk` is logged at WARNING every minute. It is noise and should be DEBUG, or logged once per change.
