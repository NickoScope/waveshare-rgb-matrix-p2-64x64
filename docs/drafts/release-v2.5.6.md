...
Smaller things: the portal's list of pages no longer shows empty rows for unused effect slots. Lua effects and market pages are grouped under one switch each, since that is how the panel turns them on and off. For people using the tools in tools/agent, the health self-test now checks the new behaviour.
...
Мелочи: в списке страниц портала больше нет пустых строк от незанятых слотов эффектов. Lua-эффекты и страницы рынков собраны под одним переключателем на группу — панель и включает их так. Для инструментов в tools/agent самотест теперь проверяет новое поведение.
