...
**Both are in the model since 2026-09-16** (`kConfirm = 2`, and a speed limit `kMaxSpeedMmS = 2000` with a 300 ms floor). The first attempt used a fixed 1.5 m step and the audit rejected it: the largest real step measured here is 1,421 mm in about 1.07 s, only 5 % under that threshold, and since a rejection also restarts the slot's proof, a target whose samples kept jumping would never have been drawn at all. A speed times the elapsed time leaves the fastest real step 40 % of room and still catches the ghost's 2 m jumps. The same audit found that a predecessor older than the freshness window was still being interpolated across, which past about 142 s overflowed the arithmetic; that is fixed too, and `tools/luasim/presence_panel.py` applies both rules so the previews keep matching the firmware.

**The preview twin drifted once, and that is worth remembering.** The commit said it applied both rules; it applied one. The Python noticed a slot going empty only when a message said so explicitly, not when the feed simply went quiet, so on a synthetic eight-second stall the preview drew a line across the pause while the firmware drew nothing. The audit caught it by running that synthetic rather than by reading the diff. Whenever a rule changes in `src/presence/presence_model.h`, the twin has to change with it, and the check is a stall and a jump fed through both, not a reading of the code. The thresholds come from the measurements above: a person's step is 40 mm median and 537 mm at p95, the ghost jumped over a metre 25 times and as far as 3 m. The price is recorded in the host tests: a target returning after an absence appears one message later, about a second.
