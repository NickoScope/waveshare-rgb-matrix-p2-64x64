...
**НЕ ПРОВЕРЕНО по официальной схеме.** Значения взяты из рабочего конфига
`packages/controllers/waveshare-esp32-s3-rgb-matrix.yaml` репозитория hub75-studio
(MIT, © 2026 Wade Edwards). Официальная схема лежит в `hardware/schematics`
репозитория `waveshareteam/ESP32-S3-RGB-Matrix`, сверить перед пайкой чего-либо своего.
...
Из того же источника, с той же оговоркой.
