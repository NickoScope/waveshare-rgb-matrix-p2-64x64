...
def main():
    ...
    ff = load("case_body.stl", SHELL_F)
    bs = load("back_cover.stl", SHELL_B)
    mk = load("mockups.stl", MOCK)

    panels = [
        ("Корпус · с лица",
         [ff], 18.0, 208.0, True,
         "окно с фаской 70°, отверстие втулки энкодера"),
        ("Корпус · изнутри",
         [ff], 18.0, 26.0, False,
         "борта, четверть под крышку, 12 бобышек под втулки M3, канавка шнура"),
        ("Задняя крышка · снаружи",
         [bs], 18.0, 26.0, False,
         "12 винтов с зенковкой заподлицо, прилив под колодки"),
        ("Задняя крышка · изнутри",
         [bs], 18.0, 208.0, True,
         "стойки матриц по сетке M3, площадка контроллера, кабельный ввод"),
        ("Макеты железа · сзади",
         [mk], 18.0, 26.0, False,
    ...
    cuts = [
        ("Разрез E — узел притяжки крышки",
         "E_join", 90.0, 0.0, False,
         "бобышка корпуса, втулка M3, винт сзади заподлицо"),
