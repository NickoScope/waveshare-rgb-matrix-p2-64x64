...
зазоров. Каждый разрез — пересечение сборки с полупространством.
    ...
    F  полный разрез    — вертикальная плоскость через контроллер, вся высота
...
SECTIONS = {
    ...
    # Полный разрез: вертикальная плоскость через контроллер, без окна —
    # вся высота панели целиком, со всем, что внутри.
    "F_full": ("x", 60.0, True, (-BIG, BIG, -BIG, BIG)),
}
